...
### §7.1 2차 USER DIRECTION 2026-05-16 — 6 expansion-layer 축 등록 + **코드 구현**

§7의 expansion 레이어에 6개 축을 추가 등록하고 **실제로 코드 구현**(per-axis
`.tape` + axis-verb `.hexa` + 결정론적 real-limits Python bridge + output
schema)했습니다. 6개 전부 sentinel PASS · `selftest/run_all.sh` 게이트 배선 ·
`hexa verify` 🟢-class **SUPPORTED-NUMERICAL** (인용된 real-limit과 일치하는
결정론적 수치 recompute).

| 항목 | 내용 |
|---|---|
| **신규 Expansion-MAIN** | **METALLODRUG** (CFSE/ligand-field + cisplatin Pt–N7 ~2.0 Å 앵커; cisplatin/carboplatin/oxaliplatin/auranofin/As₂O₃) · **OLIGONUCLEOTIDE** (SantaLucia NN ΔG/Tm; nusinersen/patisiran/inclisiran) |
| **신규 Sub :> COVALENT** | REVERSIBLE-COVALENT (Eyring TST; nirmatrelvir 가역 nitrile vs 비가역 ibrutinib) |
| **신규 Sub :> RIBOZYME** (core, 불변) | RNA-TARGETING-SMALL-MOLECULE (Nussinov 앙상블 shift; risdiplam) · APTAMER (folding ΔG + Langmuir Kd; pegaptanib) |
| **신규 Sub :> VIROCAPSID** (core, 불변) | CAPSID-ASSEMBLY-MODULATOR (Caspar-Klug 기하 + Zlotnick kinetic-trap; lenacapavir) |
| **UNPLACED** (정직, 코드축 아님) | THERANOSTIC (기존) · **GENETIC-MEDICINE** · **ADC** — 둘 다 CBER scope 결격(criterion #4 drug-only/CDER), 강행 시 g8 위반이므로 코드 미구현 |
| **기록 위치** | [`HIERARCHY.tape`](HIERARCHY.tape) §1/§2 + Log (core-5 불변·cross-link) |

**정직 caveat**: METALLODRUG·OLIGONUCLEOTIDE는 §3 Top-5 brainstorm 후보가
**아니라** 2026-05-16 세션에서 assistant가 제안한 후보 — M/N/R/S/F 점수는
출처 전사가 아닌 assistant 추정치(§6 subjective-rubric caveat 동일 적용).
§4 keep-5 엄밀 dissent 여전히 보존. 모든 PASS = in-silico
simulator-consistency only (`g8`/`f2`). 외부 modality는 자체 약물 precedent
로만 기술(`g3`/`f1`), n=6 lattice 비도출(`f_lattice_fit`). 200-disease
재맵핑·falsifier 재정의 미실행.
...
- 2026-05-16 — **§7.1 추가 — 2차 USER DIRECTION: 6 expansion-layer 축
  ...
  Python bridge + schema), 신규 sub REVERSIBLE-COVALENT(:>COVALENT)·
  RNA-TARGETING-SMALL-MOLECULE+APTAMER(:>RIBOZYME)·CAPSID-ASSEMBLY-
  MODULATOR(:>VIROCAPSID). 6개 전부 sentinel PASS·run_all.sh 게이트
  배선·hexa verify 🟢-class SUPPORTED-NUMERICAL. HIERARCHY.tape §1/§2/Log
  에 등록. GENETIC-MEDICINE·ADC는 CBER scope 결격(criterion #4) →
  UNPLACED(코드 미구현, 정직). METALLODRUG·OLIGONUCLEOTIDE는 §3 Top-5가
