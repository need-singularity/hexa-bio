...
| **ribozyme** | catalysis | 🟡 STRUCTURAL-EXACT-**CANDIDATE** (12-nt) — deductive PASS via §11 verifier | 🟡 F-RB-2 n=30 log_bf 79.74 PASS, deterministic-rubric 하 PASS | ✅ `ribozyme_output_v1` MFE inline port LANDED 2026-05-12 (Nussinov, R-R1 closed) + off-target screen real impl LANDED 2026-05-12 (G26-RB-3 comp 3) — stub allowance DEPRECATED | 4/4 | **🟢 ~95% — 잔여: G26-RB-3 full host-transcriptome corpus + G26-RB-1′ rubric sim 재실행 (둘 다 out-of-repo, 값 이미 MVP — expected PASS)** |
...
**닫힘 조건**: G26-RB-1′ computational verification rubric 7/7 PASS (재현가능 — sim 재실행 formality, 값 이미 MVP) + R-R1 stub 제거 (✅ 2026-05-12) + G26-RB-3 in-repo (✅ 2026-05-12) + G26-RB-2 branch-lock (✅ 2026-05-12) + C2 4/4 (이미) → ribozyme v1.x closure-grade. **human-rater 의존성 없음.** in-repo 부분 완결; 잔여 = sim 재실행 + full transcriptome corpus (out-of-repo).
