...
| **nanobot** | actuation | 🟡 STRUCTURAL-EXACT-**CANDIDATE** (12-vertex) — deductive PASS via §11 verifier | 🟡 n=60 curated log10_BF 13.65 decisive, deterministic-rubric 하 PASS | ✅ `actuator_output_v1` v2 reference emitter LANDED 2026-05-12 (in-repo N-R1 closed) + L6 handoff schema sealed producer-side (N-R2 = canon canonical session 대기, out-of-repo) | 4/4 | **🟢 ~98% — N-R1 v2 emit ✅ · C0d cuboctahedron dual-skeleton sim 재실행 ✅ · **N-R2 hexa-bio-side LOCKED v1.0.0 ✅ 2026-05-12** (`handoff_l6_emission_v0.schema.json` lock_metadata, `emission_blocked_until_schema_lock=false`, verified consistent w/ canon@mk1 `raw_77_therapeutic_nanobot_l7_acceptance_v1` DECLARED; vendored ref `nanobot/spec/canon_l7_acceptance_handoff_ref.json`; F-NB-1-c ratio 0.0 PASS); 잔여 = canon-side wet-lab/IP (canon cycle-30+, not a v1.x blocker) — 소소 robustness only** |
...
| **quantum** | computation | 🟢 VERIFIED (H₂ 6-Pauli / LiH path) + **pocket-scale ✅** (F-Q-6-D) + **library-ranking ✅** (F-Q-6-F) | F-Q-1…5 PASS · F-Q-EXT-1…6+ PASS · **F-Q-6-D PASS** (Mpro pocket cluster, sub-µHa) · **F-Q-6-F PASS** (5-warhead VQE library ranking, all 11 fragments VQE=CASCI sub-µHa) | (n/a) | (n/a) | **🟢 ~83% — F-Q-6 / L3 ✅ + Phase D / F-Q-6-F ✅ CLOSED 2026-05-12; L4 single-residue subsumed; GATE-26-2 — canon@mk1 state absorbed 2026-05-12: `lean4-n6/N6/` Theorem B (σ·φ=n·τ⟺n=6) ESSENTIALLY FULLY PROVEN (~4473 ln, ~2 sorry, ~99.99%); `formal/lean4/` 4-axis STUB LANDED (4-sorry, cycle 30+); hexa-bio `lean4_proof_witness_emit.py` re-impl + `canon_lean4_state_ref.json` vendored + run_all.sh-wired; Π¹₁-CA₀ label re-scoped → `decide`/RCA₀-level for the finitary slice (proof bodies = cycle 30+, in canon)** |
...
4. ~~**QUANTUM** — long pole~~ — **F-Q-6 / L3 ✅ + Phase D / F-Q-6-F ✅ CLOSED 2026-05-12** (`tests/mpro_pocket_vqe_v7.py` + `tests/mpro_warhead_library_vqe_v7.py`; all VQE=CASCI sub-µHa). L4 single-residue subsumed. **GATE-26-2 — canon@mk1 state ABSORBED 2026-05-12**: canon `lean4-n6/N6/` Theorem B (σ·φ=n·τ⟺n=6) is ESSENTIALLY FULLY PROVEN (~4473 ln, sorry-count ≈ 2, ~99.99% coverage — the n=6 mathematical foundation is machine-verified); canon `formal/lean4/` is a 4-axis STUB LANDED (4-sorry skeleton, proof bodies cycle 30+); hexa-bio absorbed it via `weave/spec/canon_lean4_state_ref.json` + a re-implemented `_python_bridge/module/lean4_proof_witness_emit.py` (emits the 4 `raw_77_lean4_proof_witness_v0` rows, run_all.sh-wired) + a note in `weave/spec/lean4_mechanical_layer_v0.scaffold.md`; the Π¹₁-CA₀ label is re-scoped → for the *finitary* axis-claims (|S₄|=24, σ(6)=12, master identity) the right target is a Lean `decide`/RCA₀-level lemma (mathlib has `Fintype.card_perm` etc.) — a *complete* proof, stronger than a sorry stub. So GATE-26-2 = (uniqueness ✅ proven · 4-axis STUB ✅ landed · consumer witness-emit ✅ re-impl · finitary slice = decide-level, cheap) — only the 4 WEAVE-mechanical proof bodies + the MechVerif sorries remain (cycle 30+, in canon — hexa-bio holds no `.lean` files by design). (F-Q-6-D *E_int* line — `.roadmap.quantum` rows 96-103 — still software-blocked on D3/basis; not a closure deliverable.)
