...
| **virocapsid** | assembly | ✅ STRUCTURAL-EXACT (T=1 corpus n=10 post 1.0 / log10_BF 16.63 · multi-T T=3/T=4 yield ≥0.85) | ✅ decisive (PDB corpus + calibration) | ✅ `cage_output_v1` LOCKED 2026-05-12 (GATE-26-V-R1 / C5 in-repo CLOSED, 4-fixture conformance PASS) | 4/4 | **🟢 ~99% (v1.x closure-grade, in-repo 완전 종료) — C5 schema lock ✅ · C3a + C3b (GATE-26-V-1b) CLOSED in-repo ✅ 2026-05-12 via VIPERdb v3.0 web-service snapshot (n=527 / 87 families / 15 T-strata; log10_BF 876.27; 7/7 C3a + 3/3 C3b PASS); `virocapsid_pdb_corpus.py` re-landed + `--refresh-viperdb` + run_all.sh-wired; 잔여 = F-VIROCAPSID-1-c/-d 독립축 (소소; cycle 28+)** |
...
| **ribozyme** | catalysis | 🟡 STRUCTURAL-EXACT-**CANDIDATE** (12-nt) — deductive PASS via §11 verifier | 🟡 F-RB-2 n=30 log_bf 79.74 PASS, deterministic-rubric 하 PASS | ✅ `ribozyme_output_v1` MFE inline port LANDED 2026-05-12 (Nussinov, R-R1 closed) + off-target screen real impl LANDED 2026-05-12 (G26-RB-3 comp 3) — stub allowance DEPRECATED | 4/4 | **🟢 ~98% — in-repo closure 완결 (R-R1 ✅ · G26-RB-2 ✅ · G26-RB-3 in-repo ✅ incl. off-target pool EXTENDED to n=206 via GENCODE v47 snapshot 2026-05-12 · G26-RB-1′ sim 재실행 ✅); 잔여 = G26-RB-3 *FULL* ~250k-transcript GENCODE/RefSeq screen w/ RIsearch2-grade scoring — needs a real aligner + a ~50 MB download → documented external step (`--gencode-pipeline-doc`), not a v1.x closure blocker** |
| **quantum** | computation | 🟢 VERIFIED (H₂ 6-Pauli / LiH path) + **pocket-scale ✅** (F-Q-6-D) + **library-ranking ✅** (F-Q-6-F) | F-Q-1…5 PASS · F-Q-EXT-1…6+ PASS · **F-Q-6-D PASS** (Mpro pocket cluster, sub-µHa) · **F-Q-6-F PASS** (5-warhead VQE library ranking, all 11 fragments VQE=CASCI sub-µHa) | (n/a) | (n/a) | **🟢 ~82% — F-Q-6 / L3 ✅ + Phase D / F-Q-6-F ✅ CLOSED 2026-05-12 (`tests/mpro_pocket_vqe_v7.py` + `tests/mpro_warhead_library_vqe_v7.py`); 잔여: L4 single-residue (pocket 에 subsumed) + GATE-26-2 lean4 → v2.0.0** |
...
3. ~~**VIROCAPSID**~~ — σ(6) 이미 STRUCTURAL-EXACT; **C5 schema lock ✅ + C3a Bayesian audit re-impl ✅ + C3b (GATE-26-V-1b) CLOSED in-repo ✅ 2026-05-12** (VIPERdb v3.0 snapshot n=527 / 87 families / 15 T-strata, log10_BF 876.27, 7/7 C3a + 3/3 C3b PASS; `virocapsid_pdb_corpus.py` + `--refresh-viperdb`). 잔여 = F-VIROCAPSID-1-c/-d 독립축 (소소; cycle 28+). **in-repo 완전 종료.**
4. ~~**QUANTUM** — long pole~~ — **F-Q-6 / L3 ✅ CLOSED 2026-05-12** (`tests/mpro_pocket_vqe_v7.py` — Mpro [Cys145 thiolate + His41 imidazolium + nirmatrelvir nitrile] pocket cluster, 2e/2o → 2 qubit VQE → sub-µHa 0.0001 µHa vs CASCI(2,2)) **+ Phase D / F-Q-6-F ✅ CLOSED 2026-05-12** (`tests/mpro_warhead_library_vqe_v7.py` — 5-warhead covalent-Mpro-inhibitor library ranked by gas-phase model ΔE_rxn at 2e/2o → 2 qubit VQE vs CASCI(2,2); all 11 fragments VQE=CASCI sub-µHa; ranking α-ketoamide < CF3-ketone < aldehyde < Michael < nitrile — nitrile least reactive ⇒ why nirmatrelvir’s nitrile is *reversible*). 잔여: L4 single-residue active site (subsumed by the pocket cluster), GATE-26-2 lean4 cert → v2.0.0. (The parallel F-Q-6-D *interaction-energy* (E_int) line — `.roadmap.quantum` rows 96-103 — is a different, harder thing, still software-blocked on D3/basis; F-Q-6/L3 and F-Q-6-F are the *closure* deliverables and they PASS.)
...
- [ ] (external) FULL transcriptome off-target screen: `gencode.v47.transcripts.fa.gz` (~250k) + RIsearch2 (Alkan *NAR* 45:e60 2017) / Cas-OFFinder / bowtie + ΔG/accessibility-weighted scoring + NHH-triplet adjacency — needs a real aligner + a ~50 MB download (not vendored); see `docs/closure_100_research_2026_05_12.md` §B.
...
**닫힘 조건**: G26-RB-1′ rubric 9/9 + **sim 재실행 ✅** + R-R1 stub 제거 (✅) + G26-RB-3 in-repo (✅, off-target pool n≈206 GENCODE-extended) + G26-RB-2 branch-lock (✅) + C2 4/4 (이미) → **ribozyme v1.x closure-grade REACHED, in-repo 완결.** **human-rater 의존성 없음.** 잔여 = G26-RB-3 *FULL* ~250k-transcript GENCODE/RefSeq screen w/ RIsearch2-grade scoring (documented external step — needs a real aligner + a ~50 MB download; `--gencode-pipeline-doc`; NOT a v1.x closure blocker).
...
**닫힘 조건**: F-Q-6 / L3 (Mpro Cys145+His41 pocket VQE) ✅ + Phase D / F-Q-6-F (5-warhead library ranking) ✅ + L4 single-residue (subsumed) + (F-Q-EXT-7b / L1 BeH₂ — minor) → **quantum v1.x closure-grade essentially REACHED 2026-05-12, in-repo**. 잔여 = GATE-26-2 lean4 cert (v2.0.0; per 2026-05-12 research the right strength is a `decide`/RCA₀-level Lean cert, not Π¹₁-CA₀).
...
- **nanobot N-R2** — L6 handoff schema 의 `emission_blocked_until_schema_lock`
  ...
  참조; §4 표 참조.)*
- **ribozyme G26-RB-3 FULL transcriptome screen** — in-repo pool EXTENDED 2026-05-12 to n≈206 (GENCODE v47 pc-transcript subset n=200 vendored, `ribozyme/spec/human_transcript_pool_snapshot.json`; `--refresh-gencode` / `--full-pool` / `--gencode-pipeline-doc`). The *FULL* ~250k-transcript GENCODE/RefSeq screen with RIsearch2-grade ΔG/accessibility scoring (Alkan *NAR* 45:e60 2017) + NHH-triplet adjacency genuinely needs a real (non-stdlib) aligner + a ~50 MB download → documented (`--gencode-pipeline-doc`), not vendored. (The earlier 'off-target screen' was a 6-mRNA toy; now 206.) ~~off-target screen 의 *full host-transcriptome*~~
...
| 2 | **RIBOZYME G26-RB-3** — full host-transcriptome off-target screen | 🟡 **PARTIALLY DONE 2026-05-12 — in-repo pool extended; FULL screen = documented external step.** Executed: `ribozyme_off_target_screen.py` now ships a vendored **GENCODE v47 pc-transcript subset n=200** (`ribozyme/spec/human_transcript_pool_snapshot.json`, first 200 records truncated 400 nt; `--refresh-gencode` rebuilds from the live FTP), `--full-pool` runs the Hamming screen against {6-mRNA core toy + (CUG)ₙ decoy + 200 GENCODE} = 206 sequences / 80.5 kb (legit arms clean), and `--gencode-pipeline-doc` prints the FULL-transcriptome pipeline (download `gencode.v47.transcripts.fa.gz` ~250k + RIsearch2/Cas-OFFinder/bowtie + ΔG/accessibility scoring + NHH-triplet adjacency — refs Alkan *NAR* 45:e60 2017, Damle *Nucleic Acid Ther.* 35:249 2025, Werner & Uhlenbeck *NAR* 23:2092 1995). **The FULL ~250k-transcript screen with RIsearch2-grade scoring genuinely needs a real (non-stdlib) aligner + a ~50 MB transcriptome download → documented, not vendored** (a stdlib-only Hamming scan over 250k transcripts is too slow + methodologically too weak — no G·U wobble / accessibility). | (in-repo as far as it goes) — the remaining FULL screen: `wget gencode.v47.transcripts.fa.gz` + a RIsearch2 (or vendored Cas-OFFinder binary) wrapper around the in-repo arm sequences. |
| 3 | **NANOBOT N-R2** — L6→L7-9 cross-repo interface contract | **Different-repo + small engineering, not a research gap.** Patterns: versioned JSON Schema artifact (version-embedded `$id`); semantic versioning of the data contract; consumer-driven (Pact-style) contract tests in *both* repos' CI; schema-registry compatibility discipline; RO-Crate/Bioschemas if provenance must travel. | Define the L6-witness JSON Schema w/ a versioned `$id`, commit a golden-example witness, add CI validation in `hexa-bio` (producer) + a consumer-driven contract test in `canon` (L7-9). |
| 4 | **QUANTUM GATE-26-2** — "Π¹₁-CA₀ certificate" in Lean 4 | **v2.0.0, small — and the spec label is mis-calibrated.** Every claim (\|S₄\|=24, σ(6)=12, S₄≅O, V−E+F=2, Eyring formula, master identity) is **finitary/decidable** → the appropriate target is a **Lean 4 `decide`/`Decidable`-backed cert (formal strength ≤ RCA₀ ≈ PRA)**; Π¹₁-CA₀ (the strongest "Big Five", fully impredicative — Simpson, *Subsystems of 2nd-Order Arithmetic*, 2nd ed. 2009) is absurd overkill for "4!=24". mathlib has `Fintype.card_perm` (⇒ \|S₄\|=24), `Nat.ArithmeticFunction.sigma` (⇒ σ 1 6=12), `Nat.totient`. **Missing in mathlib**: "cube-rotation-group ≅ S₄" (~few-hundred-line exercise), "V−E+F=2 for convex polyhedra" (port from Mizar/HOL — biggest item). Caspar-Klug "12 pentamers" — *no* formalization in any prover (would be a first; formalizable). | v2.0.0 stub: `theorem card_S4 : Fintype.card (Equiv.Perm (Fin 4)) = 24 := by simp [Fintype.card_perm]` + σ(6)=12 / master identity by `decide`/`norm_num`; **re-scope the GATE-26-2 wording from "Π¹₁-CA₀" to "Lean `decide` / RCA₀-level certificate"** (kept the legacy term in `.roadmap.hexa_bio §G` / §A.3 for traceability — the re-scope is a recommendation, not a unilateral rename). |
