...
| **ribozyme** | catalysis | 🟡 STRUCTURAL-EXACT-**CANDIDATE** (12-nt) — deductive PASS via §11 verifier | 🟡 F-RB-2 n=30 log_bf 79.74 PASS, deterministic-rubric 하 PASS | ✅ `ribozyme_output_v1` MFE inline port LANDED 2026-05-12 (Nussinov, R-R1 closed) + off-target screen real impl LANDED 2026-05-12 (G26-RB-3 comp 3) — stub allowance DEPRECATED | 4/4 | **🟢 ~99% — in-repo closure 완결 (R-R1 ✅ · G26-RB-2 ✅ · G26-RB-1′ sim 재실행 ✅ · G26-RB-3 ✅ — off-target pool n=206 via GENCODE v47 snapshot + **FULL GENCODE v47 pc-transcriptome screen EXECUTED 2026-05-12 via RIsearch2 v2.1** (summary vendored `ribozyme/spec/gencode_v47_offtarget_risearch2_summary.json`)); 잔여 = F-VIROCAPSID-1-style 소소 robustness only — no v1.x closure blocker** |
...
**닫힘 조건**: G26-RB-1′ rubric 9/9 + **sim 재실행 ✅** + R-R1 stub 제거 (✅) + **G26-RB-3 ✅** (off-target pool n≈206 GENCODE-extended + FULL GENCODE v47 pc-transcriptome RIsearch2 screen EXECUTED 2026-05-12, summary vendored) + G26-RB-2 branch-lock (✅) + C2 4/4 (이미) → **ribozyme v1.x closure-grade REACHED, in-repo 완전 종료.** **human-rater 의존성 없음.** 잔여 = 소소 robustness only (no v1.x closure blocker).
