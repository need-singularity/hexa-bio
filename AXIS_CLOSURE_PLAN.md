...
| **virocapsid** | assembly | ✅ STRUCTURAL-EXACT (T=1 corpus n=10 post 1.0 / log10_BF 16.63 · multi-T T=3/T=4 yield ≥0.85) | ✅ decisive (PDB corpus + calibration) | ✅ `cage_output_v1` LOCKED 2026-05-12 (GATE-26-V-R1 / C5 in-repo CLOSED, 4-fixture conformance PASS) | 4/4 | **🟢 ~98% (v1.x closure-grade, in-repo 완결) — C5 schema lock ✅ · C3a Bayesian audit re-impl + extended to n=35 ✅ 2026-05-12 (`virocapsid_pdb_corpus.py` re-landed, log10_BF 58.20, 7/7 PASS, run_all.sh-wired); 잔여 = C3b *full* n≥100 corpus (cycle-28+ robustness stretch, NOT a closure blocker)** |
...
3. ~~**VIROCAPSID**~~ — σ(6) 이미 STRUCTURAL-EXACT; **C5 schema lock ✅ + C3a Bayesian audit re-impl + n=35 확장 ✅ 2026-05-12** (`virocapsid_pdb_corpus.py` re-landed). 잔여 = C3b *full* corpus n≥100 + posterior ≥0.95 (cycle-28+ robustness stretch, NOT a v1.x closure blocker) + F-VIROCAPSID-1-c/-d 독립축. **in-repo 완결.**
...
| GATE-26-V-1b (C3b) | F-VIROCAPSID-1 *full* corpus — n≥100 PDB entries + posterior ≥ 0.95 (현재 n=35 in-repo, post 1.0, log10_BF 58.20) | 2027-04-28 (deferred cycle 28+) | hexa-bio session | 🟡 infra re-landed in-repo 2026-05-12 (`virocapsid_pdb_corpus.py`, n=35; `--refresh` RCSB), n≥100 확장 = exhaustive curation 대기 (robustness, NOT a closure blocker) |
...
- [ ] C3b *full* corpus 확장: RCSB PDB API 로 n≥100 (T strata ≥3, source class ≥2 유지) + Bayesian re-audit posterior ≥ 0.95 — exhaustive T-number curation across the structural record (cycle-28+ robustness stretch, NOT a v1.x closure blocker).
...
| `virocapsid_pdb_corpus` | `_python_bridge/module/virocapsid_pdb_corpus.py` | virocapsid **C3a Bayesian audit re-run + n=35 extension** (icosahedral-capsid corpus + Bayes σ(6)=12 vs uniform{5..50}; log10_BF 58.20, 7/7 sub-criteria PASS; re-impl of R5-sunset original; `--refresh` RCSB) | `__VIROCAPSID_PDB_AUDIT__ PASS` |
...
|---|-----|---------|--------------------------------|
| 1 | **VIROCAPSID C3b** — n≥100 capsid corpus w/ T-numbers | **Closeable in-repo.** The corpus (~900 entries) exists in **VIPERdb v3.0** (Montiel-Garcia et al., *NAR* 49:D809, 2021; PMC7779063) — T-number is a first-class field, sortable "T-number Index" tables, Web API (CSV/JSON). RCSB has icosahedral symmetry + stoichiometry but *not* T-number; EMDB has point-group but not T. "12 pentamers ∀T" = a Caspar-Klug theorem → audit needs only the (PDB-ID, T) list. | Pull the VIPERdb v3.0 T-number/family tables (https://viperdb.org → Data, or the WAPI) → (PDB-ID, family, T) for ≥100 (≥800); cross-check a sample vs RCSB `rcsb_struct_symmetry`; re-run the Bayesian audit. |
| 2 | **RIBOZYME G26-RB-3** — full host-transcriptome off-target screen | **Closeable in-repo *only if a real (non-stdlib) dependency is accepted.*** Credible screen = `{GENCODE v47/RefSeq transcript FASTA} + {RIsearch2 (preferred, Alkan et al. *NAR* 45:e60, 2017) or Cas-OFFinder or bowtie -v3} + {ΔG/seed/accessibility-weighted scoring} + {NHH-triplet adjacency filter}`. A pure-Python Hamming scan over ~250k transcripts is too slow *and* methodologically too weak (no G·U wobble / accessibility) to honestly call "full transcriptome screen." | Thin wrapper: download `gencode.v47.transcripts.fa.gz` (ftp.ebi.ac.uk/pub/databases/gencode/…) + shell out to RIsearch2 (rth.dk/resources/risearch/); keep the 6-mRNA toy as a unit test. |
...
| 5 | **QUANTUM Phase D (F-Q-6-F)** — 5–10-candidate VQE library ranking | **Closeable in-repo, as an extension of the existing Mpro/nirmatrelvir (2,2)→2-qubit demo.** Literature-aligned (Li et al. arXiv:2401.03759 — VQE+QM/MM on a covalent KRAS-G12C inhibitor; IBM-Merck kinase H-bond VQE; active-space-VQE drug-molecule benchmark arXiv:2512.18203; survey arXiv:2408.13479) *iff* scoped as **relative** reaction/binding energies in **minimal active spaces** with **CASCI validation on the same space** — never a standalone affinity engine or a quantum-advantage claim. Mpro mechanism confirmed (Owen et al. *Science* 374:1586, 2021; Zhang et al. *Science* 368:409, 2020; Jin et al. *Nature* 582:289, 2020; Ramos-Guzmán et al. *JACS* 145, 2023): Cys145-thiolate (His41-deprotonated) ⇄ nitrile thioimidate; Gly143/Ser144/Cys145 oxyanion hole; His163/Glu166 S1; P1 (S)-γ-lactam glutamine surrogate. | Build a 5–10-member congeneric warhead/P-substituent series around nirmatrelvir; per-member (2,2) (or (4,4)) active space at the Cys-Sγ/warhead-π* region; VQE + CASCI(same space) for the gas-phase model reaction energy; report ΔΔE_rxn ranking + the caveat "VQE = CASCI here — a CASCI ranking with a quantum-algorithm wrapper, not a quantum-advantage result." |
