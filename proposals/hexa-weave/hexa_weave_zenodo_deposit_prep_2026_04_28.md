...
deprecated_reason: papers CLI delegation (papers/.own)
...
> **Governance gates**: paper-publication-tier (explicit user approval); paper-DOI mandate (Zenodo automatic DOI satisfies); C3 honest disclosure (mechanical vs empirical); (no Clay Millennium claim); (21-section completeness preserved by parent paper); PII-safe (no sensitive personal data committed without user approval).
...
| 2 | DOI (Zenodo automatic) | AUTOMATIC on deposit | Satisfies paper-DOI mandate the moment deposit is submitted. No pre-allocation required. |
...
- **(no Clay Millennium)**: re-checked in parent paper §20.2; Robin 1984 cited without invoking the RH-equivalent sharp form.
- **C3 (mechanical vs empirical honesty)**: parent paper §20.2 explicitly enumerates the seven named axioms and the zero-empirical-claim posture. No protein-folding empirical claims migrate into the deposit.
- **(paper-DOI mandate)**: Zenodo automatic DOI satisfies this on deposit.
- **(paper-publication-tier governance)**: Option-A requires explicit user approval; this proposal does NOT initiate deposit.
- **(English primary)**: parent paper is English; this proposal is bilingual where helpful but English-primary.
...
## §5 Five falsifiers (Zenodo-prep tier)
