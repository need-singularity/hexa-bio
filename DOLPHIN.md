...
domain: dolphin
requires:
  - to: dolphin-bioacoustics
  - to: neuroscience
  - to: ecology
---

<!-- @own(sections=[WHY, COMPARE, REQUIRES, STRUCT, FLOW, VERIFY, EVOLVE], strict=false, order=sequential, prefix="§") -->
# Ultimate Dolphin Acoustic Architecture (HEXA-DOLPHIN) — n=6 echolocation / sonar / BCI / language / society / conservation integration

## §1 WHY (how this technology changes your life)

Dolphin click frequency 60–120 kHz = σ·sopfr~σ²·τ band.
**dolphin domain pattern: three prior limits addressed concurrently by the n=6 draft architecture.**
...
|------|------|-----------|----------|
| Sonar resolution cm | 10 | **2** | felt: φ=2 link |
| Frequency band kHz | 20 | **120** | felt: σ²·sopfr/6 link |
| Language pattern ID | 3 | **12** | felt: σ=12 link |
| BCI channels | 2 | **6** | felt: n=6 link |

**One-line summary**: Dolphin click frequency 60–120 kHz = σ·sopfr~σ²·τ band — the n=6 perfect-number draft architecture addresses frequency band improvement and three prior limits concurrently.

### When it becomes everyday

```
  [dolphin] once data/resource/infrastructure is aligned to the n=6 structure
...
|------|------|---------|
| Productivity | Frequency-band target 120 kHz | σ·sopfr=60 |
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│  [Ultimate Dolphin Acoustic Architecture (HEXA-DOLPHIN) performance] baseline vs HEXA performance comparison    │
├──────────────────────────────────────────────────────────────────────────┤
│  Sonar resolution cm
│  Baseline  ████████████████████████████  10
│  HEXA   ██████░░░░░░░░░░░░░░░░░░░░░░  2  (φ=2)
...
|-------------|------|------|------|-----------|
| dolphin-bioacoustics | 7 | 10 | +3 | dolphin bioacoustics |
| neuroscience | 7 | 10 | +3 | neuroscience |
| ecology | 7 | 10 | +3 | ecology |

Integration of the ultimate Dolphin Acoustic Architecture (HEXA-DOLPHIN) requires maturation of all three prerequisite domains. Current stage is partial (Mk.I–II).
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│                  Ultimate Dolphin Acoustic Architecture (HEXA-DOLPHIN) system structure                     │
├────────────┬────────────┬────────────┬────────────┬─────────────────────┤
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│  Ultimate Dolphin Acoustic Architecture (HEXA-DOLPHIN) — specifications                                      │
├──────────────────────────────────────────────────────────────────────────┤
│  Essence       Dolphin click frequency 60–120 kHz = σ·sopfr~σ²·τ band
│  Core DOF      n = 6
...
│  Key metric    frequency band = 120 kHz
...
Whether the ultimate Dolphin Acoustic Architecture (HEXA-DOLPHIN) is consistent with the n=6 draft pattern using only stdlib multi-layer checks. Design specs are cross-checked against number-theoretic formulas.

### Testable Predictions (10 testable predictions)

| # | Prediction | Formula | Predicted | Tier |
|---|------|------|--------|------|
| TP-1 | Frequency-band candidate optimum | σ·sopfr/10 | 120 kHz | 1 |
...
- Falsifier: if measured frequency band < 85% the formula is discarded / if EXACT ratio < 80% the draft is withdrawn / if the candidate optimum collapses under sensitivity perturbation, the convexity hypothesis is rejected
...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# series: dolphin - HEXA n=6 honesty verification (stdlib only)
#
...
# Why needed: unit consistency for claims such as frequency band=120 kHz.
DIM = {
    'M': (1, 0, 0, 0),       # kg
    'L': (0, 1, 0, 0),       # m
    'T': (0, 0, 1, 0),       # s
...
FALSIFIERS = [
    "If measured frequency band is below 85% of prediction, discard the formula",
...
Full integration. Dolphin click frequency 60–120 kHz = σ·sopfr~σ²·τ band. Reached once all three prerequisite domains mature.
